...
class ScalarParams:
  ...
  lambda_refrgb: float = 0.0
  lambda_clip: float = 1.0
  lambda_alphatv: float = 1.0  
...
@jax.jit
def compute_clip_loss_delta(text_feat, rgb, refrgb):
...
@jax.jit
def compute_clip_loss(text_feat, rgb):
  model = FlaxCLIPModel.from_pretrained("openai/clip-vit-base-patch32")
  rgb = _clip_img_preprocess(rgb) 
  # Preprocess image suitable for CLIP model 
  rgb_feat = model.get_image_features(rgb)
  rgb_feat = rgb_feat / jnp.linalg.norm(rgb_feat, axis=1)[..., jnp.newaxis]
  sim_mat = jnp.matmul(text_feat, jnp.transpose(rgb_feat, (1, 0)))
  return 1. - jnp.mean(sim_mat)
...
def train_step(model: models.EditingNerfModel,
  ...
  def _compute_loss_and_stats(model_out, batch_shape):
    ...
    # L-2 Regularization loss (refrgb)
    ...
    clip_loss = compute_clip_loss_delta(clip_text, rgb_image, refrgb_image)
    #clip_loss = compute_clip_loss(clip_text, rgb_image)
    loss += scalar_params.lambda_clip * clip_loss
    stats['loss/clip'] = clip_loss

    # Mask TV loss
    mask = jnp.reshape(model_out['mask'], (batch_shape[0], batch_shape[1], -1)) #(H, W, num_K)
    h_diff = mask[1:, ...] - mask[:-1, ...]
    w_diff = mask[:, 1:, :] - mask[:, :-1, :]
    tv_loss = jnp.mean(jnp.square(h_diff)) + jnp.mean(jnp.square(w_diff))
    loss += scalar_params.lambda_alphatv * tv_loss
    stats['loss/mask_tv'] = tv_loss
