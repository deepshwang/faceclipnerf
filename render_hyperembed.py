...
def process_iterator(tag: str,
  ...
  interp_save_dir = datasource.data_dir / 'hyper_embed_example' / '{}x'.format(datasource.image_scale) 
  ...
  for i, (item_id, batch) in tqdm(enumerate(zip(item_ids, iterator))):
    if item_id in [format(j, '06d') for j in [9, 120, 155, 285]]:
      ...
      interp_imgname = str(interp_save_dir /  "rgb_{}.png".format(item_id))
      interp_depth_imgname = str(interp_save_dir /  "depth_{}.png".format(item_id))
