...
class EditingNerfModel(nn.Module):
  ...
  # Editing configs
  hyper_mapper_mlp_cls: Callable[..., nn.Module] = modules.HyperMapperMLP
  num_K: int = 4 
  ...
  def setup(self):
    ...
    if self.use_nerf_embed: # appearance code. False by config
      self.nerf_embed = self.nerf_embed_cls(num_embeddings=self.num_nerf_embeds)
    if self.use_warp: # Uses only one reference warp embed for a single canonical space
      self.warp_embed = self.warp_embed_cls(num_embeddings=1)

    if self.hyper_slice_method == 'axis_aligned_plane':
      self.hyper_embed = self.hyper_embed_cls(
          num_embeddings=self.num_hyper_embeds)
    # Multiple hyperspace embedding for mutual extrapolation
    elif self.hyper_slice_method == 'bendy_sheet':
      self.hyper_embed = self.hyper_embed_cls(
          num_embeddings=self.num_K)
      self.hyper_sheet_mlp = self.hyper_sheet_mlp_cls()

    if self.use_warp:
      self.warp_field = self.warp_field_cls()

    # Hyper mapper mlps for `num_K` number of hyperspace embedding
    # hyper_mapper_mlps = {}
    # for k in range(self.num_K):
    #   hyper_mapper_mlps[str(k)] = self.hyper_mapper_mlp_cls()
    # self.hyper_mapper_mlps = hyper_mapper_mlps

    self.hyper_mapper_mlp = self.hyper_mapper_mlp_cls()
  ...
  def query_template(self,
    ...
    sigma = jnp.squeeze(raw['alpha'], axis=-1)
  ...
  def map_hyper_points(self, points, hyper_embed, extra_params,
    ...
    if hyper_point_override is not None:
      ...
      mask_logits = []
      # Calculate attribute-wise mask 
      # for k in range(self.num_K):
      #   mask_logits.append(self.hyper_mapper_mlp(
      #         points,
      #         hyper_embed[k],
      #         alpha=None 
      #   ))
      # mask = jnp.transpose(jnp.expand_dims(jax.nn.softmax(jnp.concatenate(mask_logits, axis=-1), axis=-1), axis=-1), axes=(2, 0, 1, 3)) # mask_logits[i].shape = (batch_size, num_samples, 1)
      # hyper_embed = jnp.stack(hyper_embed, axis=0) # hyper_embed.shape = (batch_size, num_samples, dims_embed)
      # retrieve final local slicing surface code proportioanl to the mask
      #hyper_embed = jnp.mean(mask * hyper_embed, axis=0)

      for k in range(1, self.num_K):
        ml = self.hyper_mapper_mlp(points, hyper_embed[k], alpha=None)
        mask_logits.append(ml)
      mask = jnp.transpose(jnp.expand_dims(jax.nn.sigmoid(jnp.concatenate(mask_logits, axis=-1)), axis=-1), axes=(2, 0, 1, 3)) # mask_logits[i].shape = (batch_size, num_samples, 1)
      hyper_embed_residual = jnp.stack(hyper_embed[1:], axis=0) # hyper_embed.shape = (batch_size, num_samples, dims_embed)
      # retrieve final local slicing surface code proportioanl to the mask
      hyper_embed = hyper_embed[0] + jnp.sum(mask * (hyper_embed_residual - jnp.expand_dims(hyper_embed[0], axis=0)), axis=0)   
      # Slice using the local sicing surface
      hyper_points = self.hyper_sheet_mlp(
          points,
          hyper_embed,
          alpha=extra_params['hyper_sheet_alpha'])
      return hyper_points, jnp.transpose(jnp.squeeze(mask), axes=(1, 2, 0))

    #   k=3
    #   mask_logits = self.hyper_mapper_mlp(points, hyper_embed[k], alpha=None)
    #   mask = jax.nn.sigmoid(mask_logits) # mask_logits[i].shape = (batch_size, num_samples, 1)
    #   hyper_embed_residual = hyper_embed[k] # hyper_embed.shape = (batch_size, num_samples, dims_embed)
    #   # retrieve final local slicing surface code proportioanl to the mask
    #   hyper_embed = hyper_embed[0] + mask * (hyper_embed_residual - hyper_embed[0])   
    #   # Slice using the local sicing surface
    #   hyper_points = self.hyper_sheet_mlp(
    #       points,
    #       hyper_embed,
    #       alpha=extra_params['hyper_sheet_alpha'])
    # return hyper_points, mask

  def map_points(self, points, warp_embed, hyper_embed, extra_params,
                 use_warp=True, return_warp_jacobian=False,
                 hyper_point_override=None, use_mapper=True):
    ...
    # hyper_masks.shape = (num_K, num_batches, num_samples, 1)
    hyper_points, hyper_masks = self.map_hyper_points(
        points, hyper_embed, extra_params,
        hyper_point_override=hyper_point_override)
    # Mapper Network implementation
    # if use_mapper:
    #   delta_hyper_points = self.extend_hyper_points(
    #       points, hyper_embed, extra_params, 
    #       hyper_point_override=hyper_point_override)
    #   delta_hyper_points= delta_hyper_points[..., :-1]
    #   mask = jax.nn.sigmoid(delta_hyper_points[..., -1:])
    #   hyper_points = mask * delta_hyper_points + (1. - mask) * hyper_points 
    if hyper_points is not None:
      warped_points = jnp.concatenate([spatial_points, hyper_points], axis=-1)
    else:
      warped_points = spatial_points

    return warped_points, warp_jacobian, hyper_masks
  ...
  def render_samples(self,
    ...
    # Create the warp embedding. (warp_embed = $w_{i}$)
    if use_warp:
      warp_embed = self.warp_embed(jnp.zeros((points.shape[0], 1), dtype=jnp.uint32))
    else:
      warp_embed = None
    ...
    #   elif self.hyper_use_warp_embed: # HyperNeRF official
    #     hyper_embed = warp_embed
    #   else:
    hyper_embeds = []
    for k in range(self.num_K):
      anc_embed = self.hyper_embed(k * jnp.ones((points.shape[0], 1), dtype=jnp.uint32))
      hyper_embeds.append(anc_embed)
    hyper_embed = hyper_embeds
    ...
    if hyper_embed is not None:
      if type(hyper_embed) == list:
        for i, h in enumerate(hyper_embed):
          hyper_embed[i] = jnp.broadcast_to(
              h[:, jnp.newaxis, :],
              shape=(*batch_shape, h.shape[-1]))
      else:
    ...
    # Map input points to warped spatial and hyper points.
    warped_points, warp_jacobian, hyper_masks = self.map_points(
        points, warp_embed, hyper_embed, extra_params, use_warp=use_warp,
        return_warp_jacobian=return_warp_jacobian,
        # Override hyper points if present in metadata dict.
        hyper_point_override=metadata.get('hyper_point'), use_mapper=use_mapper)
  ...
  def __call__(
    ...
    Args:
      rays_dict: a dictionary containing the ray information. Contains:
        ...
        'depth': depth prior to sample from
