...
from hypernerf import editing_models_nerfies as models
...
def _log_grads(writer: tensorboard.SummaryWriter, model: models.NerfiesEditingNerfModel,
               state: model_utils.TrainState):
  """Log histograms to Tensorboard."""
  step = int(state.optimizer.state.step)
  params = state.optimizer.target['model']
...
def main(argv):
  ...
  dummy_model = models.NerfiesEditingNerfModel({}, 0, 0)
  ...
  params = model_utils.inject_params_nerfies(ref_params, params) 
  ...
  embed_optimizer_traverser = traverse_util.ModelParamTraversal(lambda path, _: 'warp_embed' in path)

  optimizer_def = optim.MultiOptimizer((embed_optimizer_traverser, embed_optimizer_def))
