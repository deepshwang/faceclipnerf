...
import functools
from typing import Any, Optional, Tuple, Callable, Union, Tuple
from jax import lax
...
class LipDense(nn.Module):
  """A [Lipscitz] linear transformation applied over the last dimension of the input.

  Liu et al. "Learning Smooth Neural Functions via Lipschitz Regularization" (https://arxiv.org/abs/2202.08345)
  ...
  kernel_init: Callable[[Any, Shape, Any], Array] = lecun_normal() 
  bias_init: Callable[[Any, Shape, Any], Array] = zeros
  ...
  def __call__(self, inputs: Array) -> Array:
    ...
    c = self.param('lip_c', 
                   self._c_initialize,
                   kernel)
    inputs, kernel, bias, c = promote_dtype(inputs, kernel, bias, c, dtype=self.dtype)
    
    kernel = self.L_inf_norm(kernel, jax.nn.softplus(c))
  ...
  def _c_initialize(self, rng: Any, W: Array) -> Array:
    """ Initialize learnable Lipscitz constant given a kernel matrix.

    Args:
      rng: A dummy random seed variable (initialization is deterministic w.r.t W)
      W: kernel matrix to refer for initialization
    
    Returns:
      The initial Lipscitz constant value.
    """

    return jnp.max(jnp.sum(jnp.abs(W), axis=1))


  def L_inf_norm(self, W: Array, softplus_c: Array) -> Array:
    """
...
class NerfMLP(nn.Module):
  ...
  Attributes:
    ...
    dense_fn: dense linear layer type (Normal / Lipscitz)  
  """
  trunk_depth: int = 8
  trunk_width: int = 256
  ...
  def __call__(self, x, alpha_condition, rgb_condition):
    ...
    dense = functools.partial(
        self.dense_fn, kernel_init=jax.nn.initializers.glorot_uniform())
...
@gin.configurable(denylist=['name'])
class HyperMapperMLP(nn.Module):
  """An MLP that maps a point and an embed to delta surface & its corresponding mask."""
  output_channels: int = gin.REQUIRED
  min_deg: int = 0
  max_deg: int = 1
