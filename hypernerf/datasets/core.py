...
def iterator_from_dataset(dataset: tf.data.Dataset,
  ...
  if batch_size > 0:
    ...
    dataset = dataset.batch(batch_size=abs(batch_size))
    it = map(prepare_tf_data_unbatched, dataset)
...
def _camera_to_rays_fn(item, use_tf_camera=False, square_crop=False):
  ...
  if square_crop:
      ...
      c_W = int(W//2)
      S = int(min(H, W)//2) - 1
      origins = origins[c_H-S:c_H+S+1, c_W-S:c_W+S+1, :]
      directions = directions[c_H-S:c_H+S+1, c_W-S:c_W+S+1, :]
      pixels = pixels[c_H-S:c_H+S+1, c_W-S:c_W+S+1, :]
...
class DataSource(abc.ABC):
  ...
  def create_cameras_dataset(self,
    ...
    dataset = dataset.map(
        functools.partial(_camera_to_rays_fn, use_tf_camera=True, square_crop=square_crop), _TF_AUTOTUNE)
  ...
  def create_dataset(self,
    ...
    if self.preload:
      return self._create_preloaded_dataset(
          item_ids, flatten=flatten, shuffle=shuffle, square_crop=square_crop)
    else:
  ...
  def _create_preloaded_dataset(self, item_ids, flatten=False, shuffle=False, square_crop=False):
    ...
    """
    load_fn = functools.partial(self.get_item, square_crop=square_crop)
    data_list = utils.parallel_map(load_fn, item_ids)
    c2r_fn = functools.partial(_camera_to_rays_fn, square_crop=square_crop)
    data_list = [c2r_fn(item) for item in data_list]
  ...
  def _create_lazy_dataset(self,
    ...
    cam_to_rays_fn = functools.partial(_camera_to_rays_fn, square_crop=square_crop)
    dataset = dataset.map(cam_to_rays_fn, num_parallel_calls=_TF_AUTOTUNE)
  ...
  def get_item(self, item_id, scale_factor=1.0, square_crop=False):
    ...
    """
    rgb = self.load_rgb(item_id, square_crop=square_crop)
    if scale_factor != 1.0:
