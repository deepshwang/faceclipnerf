...
def initialize_edited_warp_embed(params, batch):
  img_shape = batch['origins'].shape[:-1]
  encoded_metadata = {}
  # initialize editing warp embeds with the mean of learned warp embeds
  val = jnp.expand_dims(jnp.mean(params['warp_embed']['embed']['embedding'], axis=1), axis=0)
  val = jnp.tile(val, (img_shape[0], img_shape[1], 1))
  encoded_metadata['encoded_warp'] = val
  encoded_metadata['encoded_hyper'] = val 
...
def process_iterator(tag: str,
  ...
  depth_save_dir = datasource.data_dir / 'depth'  / '{}x'.format(datasource.image_scale) 
  refrgb_save_dir = datasource.data_dir / 'refrgb' / '{}x'.format(datasource.image_scale) 
  os.makedirs(depth_save_dir, exist_ok=True)
  os.makedirs(refrgb_save_dir, exist_ok=True) 
  meters = collections.defaultdict(utils.ValueMeter)
  for i, (item_id, batch) in tqdm(enumerate(zip(item_ids, iterator))):
    depth_filename = str(depth_save_dir / "{}.npy".format(item_id))
    depth_imgname = str(depth_save_dir / "{}.png".format(item_id))
    refrgb_imgname = str(refrgb_save_dir /  "{}.png".format(item_id))
    
    if not os.path.exists(depth_filename) or not os.path.exists(refrgb_imgname):
      ...
      jnp.save(depth_filename, depth)
      image_utils.save_image(refrgb_imgname,
                            image_utils.image_to_uint8(rgb))
      image_utils.save_image(depth_imgname,
