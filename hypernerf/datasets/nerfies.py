...
def _load_image(path: types.PathType, square_crop: bool=False) -> np.ndarray:
  ...
  with path.open('rb') as f:
    ...
    if square_crop:
      ...
      c_W = int(W//2)
      S = int(min(H, W)//2) - 1
      image = image[c_H-S:c_H+S+1, c_W-S:c_W+S+1, :] 
    return image
...
class NerfiesDataSource(core.DataSource):
  ...
  def __init__(self,
               ...
               reference_warp_id: int = None,
               reference_appearance_id: int = None,
               load_refrgb: bool = False,               
               **kwargs):
    ...
    if load_refrgb:
      self.rgb_dir = gpath.GPath(data_dir, 'refrgb', f'{image_scale}x') 
  ...
  def load_rgb(self, item_id: str, square_crop: bool=False) -> np.ndarray:
    return _load_image(self.rgb_dir / f'{item_id}.png', square_crop=square_crop)
  ...
  def load_depth(self, item_id):
    return np.load(self.depth_dir / '{}.npy'.format(item_id))
