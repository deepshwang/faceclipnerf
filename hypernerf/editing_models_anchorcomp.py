...
class EditingNerfModel(nn.Module):
  ...
  def extend_hyper_points(self, points, hyper_embed, extra_params,
    ...
    if hyper_point_override is not None:
      ...
      hyper_points = self.hyper_mapper_mlp(
          points,
          hyper_embed,
  ...
  def map_hyper_points(self, points, hyper_embed, extra_params,
    ...
    if hyper_point_override is not None:
      ...
      # # retrieve final local slicing surface code proportioanl to the mask
      # hyper_embed = hyper_embed[0] + jnp.mean(mask * (hyper_embed_residual - jnp.expand_dims(hyper_embed[0], axis=0)), axis=0)   
  ...
  def render_samples(self,
    ...
    if self.has_hyper_embed:
      if metadata_encoded:
        ...
        hyper_embeds = []
        for k in range(self.num_K):
          hyper_embeds.append(self.hyper_embed(k * jnp.ones((points.shape[0], 1), dtype=jnp.uint32)))
