...
def sample_specific_metadata(datasource, batch, step, appearance_id=None, warp_id=None):
  """Samples random metadata dicts."""
  test_rng = random.PRNGKey(step)
  shape = batch['origins'][..., :1].shape
  metadata = {}
...
def process_iterator(tag: str,
  ...
  for i, (item_id, batch) in enumerate(zip(item_ids, iterator)):
    ...
    extra_images = None
    if tag == 'test':
      batch['metadata'] = sample_specific_metadata(datasource, batch, step, appearance_id=9, warp_id=120)
      cache_batch['metadata'] = sample_specific_metadata(datasource, batch, step, appearance_id=9, warp_id=9) # 9, 120, 155, 285
    batch['metadata'] = evaluation.encode_metadata(
        model, jax_utils.unreplicate(params), batch['metadata'])
    cache_batch['metadata'] = evaluation.encode_metadata(
        model, jax_utils.unreplicate(params), cache_batch['metadata'])
...
def main(argv):
  ...
  val_eval_ids = datasource.val_ids
  if val_eval_ids:
  ...
  while True:
    ...
    #embeddings = np.squeeze(np.asarray(state.optimizer.target['model']['warp_embed']['embed']['embedding']))
    #mean_embeds, _  = kmeans(embeddings, 4)

    #anchors = []
    #for i in range(4):
    #  sim = np.sum(embeddings * mean_embeds[i:i+1, :], axis=1) / np.linalg.norm(embeddings, axis=1, keepdims=False)
    #  anchors.append(train_eval_ids[np.argmax(sim)])
    #debug.pdb.set_trace()
