...
from hypernerf import editing_models_anchorcomp as models
from hypernerf import schedules
from hypernerf import editing_anchorcomp as training
...
def main(argv):
  ...
  reference_warp_id = 9, 
  reference_hyper_id=[datasource.get_warp_id(format(i, '06d')) for i in [9, 120, 155, 285]])
  ...
  #embed_optimizer_def = optim.Adam(learning_rate_sched(0))
  #embed_optimizer_traverser = traverse_util.ModelParamTraversal(lambda path, _: 'hyper_embed' in path)

  #optimizer_def = optim.MultiOptimizer((mapper_traverser, mapper_optimizer_def), (embed_optimizer_traverser, embed_optimizer_def))
  ...
  scalar_params = training.ScalarParams(
      ...
      clip_loss_weight=train_config.clip_loss_weight)
  ...
  # Prepare clip text token
  clip_model = FlaxCLIPModel.from_pretrained("openai/clip-vit-base-patch32")
  tokenizer = CLIPTokenizer.from_pretrained("openai/clip-vit-base-patch32")
  text_inputs = tokenizer(train_config.text_prompt.split(","), padding=True, return_tensors="np")
  text_features = clip_model.get_text_features(**text_inputs)
  text_features = text_features / jnp.linalg.norm(text_features, axis=1)[..., jnp.newaxis]
  del clip_model, tokenizer, text_inputs
  ...
  for step, batch in zip(range(init_step, train_config.max_steps + 1),
    ...
    if step % train_config.save_every == 0 and jax.process_index() == 0:
      ...
      for j in range(4):
        image_utils.save_image(val_rgb_dir / "testie_{}_mask_{}.png".format(str(step), str(j)), image_utils.image_to_uint8(jnp.reshape(model_out['fine']['mask'][0, ..., j], (135, 135)))) 
